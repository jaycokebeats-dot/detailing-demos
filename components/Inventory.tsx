...
export default function Inventory({ biz }: { biz: Business }) {
  ...
  const is11Custom = isCustomList && listaServicios.length === 11;
  const grupo1 = is11Custom ? listaServicios.slice(0, 3) : [];
  const grupo2 = is11Custom ? listaServicios.slice(3, 5) : [];
  const grupo3 = is11Custom ? listaServicios.slice(5) : [];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {is11Custom ? (
          <div className="space-y-6">
            {/* Fila 1: 3 lavados/tapizados */}
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {grupo1.map((s, i) => renderCard(s, i))}
            </div>

            {/* Fila 2: 2 servicios adicionales centrados */}
            <div className="grid sm:grid-cols-2 max-w-4xl mx-auto gap-6">
              {grupo2.map((s, i) => renderCard(s, i + 3))}
            </div>

            {/* Filas 3 y 4: 6 tratamientos acrílicos/cerámicos (2 filas de 3) */}
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {grupo3.map((s, i) => renderCard(s, i + 5))}
            </div>
          </div>
        ) : (
          <div className={`grid sm:grid-cols-2 ${isCustomList ? "lg:grid-cols-3" : "lg:grid-cols-4"} gap-6`}>
            {listaServicios.map((s, i) => renderCard(s, i))}
          </div>
        )}
